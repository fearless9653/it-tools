...
| :---------------------- | :----------------------------------------------------- |
| `.` or `[^\n\r]`        | any character _excluding_ a newline or carriage return |
| `[A-Za-z]`              | alphabet                                               |
...
- The quantifier goes _after_ the expression to be quantified.
...
| `foo(?!bar)`  | match `foo` if it’s _not_ before `bar` |
| `(?<=bar)foo` | match `foo` if it’s after `bar`        |
| `(?<!bar)foo` | match `foo` if it’s _not_ after `bar`  |
...
| `(?:foo)`    | non-capturing group; match `foo` but _without_ capturing `foo`        |
